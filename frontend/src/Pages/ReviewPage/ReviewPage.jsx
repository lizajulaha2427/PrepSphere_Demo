...
const ReviewPage = () => {
  ...
  return (
    <div className="review-page">
      ...
      <div className="hero-container">
        ...
        <div className="hero-stats">
          <div className="stats-row">
            ...
            <div className="stat-box">
              <h2 className="stat-number">{stats.averageRating}</h2>
              <p className="stat-label">Avg Rating</p>
            </div>
          </div>
          
          <button onClick={() => setShowModal(true)} className="write-review-btn-hero">
            Write a Review
          </button>
        </div>
      </div>
        ...
        <div className="modall-overlay" onClick={() => setShowModal(false)}>
          <div className="modall-content" onClick={(e) => e.stopPropagation()}>
            <div className="modall-header">
              ...
              <button onClick={() => setShowModal(false)} className="modall-close">×</button>
            </div>
            <form onSubmit={handleSubmit} className="modall-form">
              <div className="form-grid">
                <div>
                  <label className="form-label">Name</label>
                  <input type="text" name="name" value={formData.name} onChange={handleFormChange} className="form-input" />
              ...
              <div className="form-full">
                ...
                <select name="course" value={formData.course} onChange={handleFormChange} className="form-select">
              ...
              <div className="form-full">
                ...
                <input type="text" name="title" value={formData.title} onChange={handleFormChange} className="form-input" />
                {formErrors.title && <p className="error-message">{formErrors.title}</p>}
              </div>
              <div className="form-full">
                <label className="form-label">Review</label>
                <textarea name="review" value={formData.review} onChange={handleFormChange} rows="4" className="form-textarea"></textarea>
      ...
      <div className="filter-section">
        ...
        <div className="filter-container">
          <div className="filter-group">
            <label className="filter-label"><Star className="filter-label-icon" />Filter by Rating</label>
          ...
          <div className="filter-group">
            <label className="filter-label"><BookOpen className="filter-label-icon" />Filter by Course</label>
            <select value={filters.course} onChange={(e) => setFilters(prev => ({ ...prev, course: e.target.value }))} className="filter-select">
          ...
          <div className="filter-group">
            <label className="filter-label"><ArrowUpDown className="filter-label-icon" />Sort By</label>
            <select value={filters.sortBy} onChange={(e) => setFilters(prev => ({ ...prev, sortBy: e.target.value }))} className="filter-select">
          ...
          <div className="filter-group">
            <label className="filter-label"><SortAsc className="filter-label-icon" />Sort Order</label>
            <select value={filters.sortOrder} onChange={(e) => setFilters(prev => ({ ...prev, sortOrder: e.target.value }))} className="filter-select">
      ...
      {/* Loading State */}
      {loading && <div className="text-center py-8"><p className="text-gray-600">Loading reviews...</p></div>}
          ...
          {paginatedReviews.map((review) => (
            <div key={review._id} className="review-card">
              <div className="review-header-card">
                <div className="avatar"><User className="avatar-icon" /></div>
                <div className="reviewer-info">
                  <span className="review-name">{review.name}</span>
                  <div className="review-stars"><StarRating rating={review.rating} /></div>
              ...
              <div className="review-footer">
                <span className="course-badge">{review.course}</span>
                <button onClick={() => handleHelpful(review._id)} disabled={updatingHelpful === review._id} className="helpful-btn">
      ...
      {!loading && paginatedReviews.length === 0 && (
        <div className="empty-state">
          ...
          <button onClick={() => setShowModal(true)} className="write-review-btn">Write the First Review</button>
        </div>
      )}

      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="flex justify-center items-center space-x-2 pagination">
